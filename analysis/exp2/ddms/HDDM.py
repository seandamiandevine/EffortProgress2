...
os.chdir('/Users/sean/documents/effprog2/exp1_local/analysis/c/')
...
plt.show()
fig.savefig(f'figs/ddm/behav_reproduce_N={dat.subject.unique().shape[0]}.pdf')
plt.close()
...
# # df['progbin'] = pd.qcut(df.progbin, 3, labels=False)
# # binlabs = np.linspace(0,1,3)
...
# plt.show()
# fig.savefig(f'figs/ddm/pars_x2_N={dat.subject.unique().shape[0]}.pdf')
...
ddm.sample(2000, burn=500, dbname=f'ddms/models/traces_quadratic_v', db='pickle')
ddm.save(f'ddms/models/quadratic_v_N={dat.subject.unique().shape[0]}')

ddm = hddm.load(f'ddms/models/quadratic_v_N={dat.subject.unique().shape[0]}')
...
plt.show()
fig.savefig(f'figs/ddm/summary_{winning_mod}.pdf')
plt.close()
...
X['showprog'] = np.where(X.showprog, 'Progress','No Progress')
...
plt.show()
fig.savefig(f'figs/ddm/{winning_mod}_parameter_predictions.pdf')
plt.close()
...
for i in tqdm(range(X.shape[0]),desc='simulating PPC...'):
    ...
    X.acc.iloc[i] = pred.response.mean().round(4)
...
plt.show()
fig.savefig(f'figs/ddm/{winning_mod}_PPC_rt.pdf')
plt.close()
...
plt.show()
fig.savefig(f'figs/ddm/{winning_mod}_PPC.pdf')
plt.close()
