...
setwd("~/Documents/EffProg2/exp1_local/analysis/c")
...
names(desc_pre$comments) = NULL; names(desc_pre$strategy) = NULL
capture.output(desc_pre,file='out/descriptives_pre.txt')
...
capture.output(desc,file='out/descriptives_post.txt')
...
macc     = macc[c(5,10),]
seacc    = seacc[c(5,10),]
yrange   = c(.5, 1)# range(pretty(c(macc-seacc-.1, macc+seacc+.1)))
...
sjPlot::tab_model(acc_lmer3, transform = NULL, file = 'out/acc_basic_glmer.html')
...
mrt     = tapply(progcor$rt, list(progbin, progcor$showprog), mean, na.rm=T) 
sert    = tapply(progcor$rt, list(progbin, progcor$showprog), se) *2
...
progcor = progcor[!is.na(progcor$subject),]
...
sert = tapply(progcor$mv_rt, list(progcor$progress, progcor$showprog), se)*2.5
mrt   = mrt[!apply(mrt, 1, function(i) any(is.na(i))),]
sert  = sert[!apply(sert, 1, function(i) any(is.na(i))),]
...
legend('topleft', bty='n', lty=1, col=c('darkgrey', 'darkgreen'), cex=1.5, title='', 
...
k   = 10
progdat = progdat[!is.na(progdat$subject),]
...
macc   = macc[!apply(macc, 1, function(i) any(is.na(i))),]
seacc  = seacc[!apply(seacc, 1, function(i) any(is.na(i))),]
yrange = range(pretty(c(macc-seacc, macc+seacc)))
colnames(macc) = c('False','True')
...
legend('bottomright', bty='n', lty=1, col=c('darkgrey', 'darkgreen'), title='Progress?',
...
cor$reward_c    = ifelse(cor$reward==0.20, .5, -.5)
...
anova(rt_q_mod0, rt_q_mod2, rt_q_mod5)

summary(rt_q_mod5)
car::Anova(rt_q_mod5)

sjPlot::tab_model(rt_q_mod5, transform = NULL, file = 'out/rt_q_mod.html')
...
dat$reward_c    = ifelse(dat$reward==.20, .5, -.5)
